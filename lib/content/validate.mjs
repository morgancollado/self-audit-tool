...
function crossRefChecks(byType) {
  ...
  // Network grouping consistency: one key must mean one display name everywhere
  // (a divergent name would render as two different "networks" for one backbone),
  // and a single-member network is a mislabel — drop the field instead.
  ...
  for (const { file, data } of byType['broker.schema.json'] ?? []) {
    ...
    if (seen && seen.name !== net.name) {
      ...
      netNames.set(net.key, { name: net.name, file });
    }
  }
