...
// the node describes each item's allowed keys (merged by index at build).
const TRANSLATABLE = {
...
function checkOverlayKeys(node, allowed, path, errors) {
  ...
  for (const [k, v] of Object.entries(node)) {
    ...
    if (!rule) {
      ...
      // Array of objects (e.g. specialMechanisms): each item's keys are checked
      // against the rule; items merge by index at build time.
      v.forEach((item, i) => checkOverlayKeys(item, rule, `${p}[${i}]`, errors));
    } else {
      checkOverlayKeys(v, rule, p, errors);
    }
  }
}
...
function overlayChecks(file, data, type, baseByPath, errors) {
  ...
  checkOverlayKeys(data, TRANSLATABLE[type], '', errors);
